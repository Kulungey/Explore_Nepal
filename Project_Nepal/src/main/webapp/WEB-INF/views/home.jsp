  ...
  <main class="section" id="destinations">
    <div class="container">
      ...
      <form action="${pageContext.request.contextPath}/home" method="get"
            style="display:flex; gap:10px; margin-bottom:var(--sp-4); max-width:480px;">
        <input type="text" name="search"
               value="${fn:escapeXml(searchKeyword)}"
               placeholder="Search destinations, regions, categories…"
               style="flex:1; padding:10px 14px; border:1.5px solid var(--color-outline);
                      border-radius:var(--radius); font-size:var(--text-base);
                      font-family:var(--font-base); outline:none;"/>
        <button type="submit" class="btn btn-primary" style="padding:10px 20px;">Search</button>
        <c:if test="${not empty searchKeyword}">
          <a href="${pageContext.request.contextPath}/home" class="btn btn-secondary" style="padding:10px 16px;">Clear</a>
        </c:if>
      </form>

      <c:if test="${not empty searchKeyword}">
        <p style="margin-bottom:var(--sp-3); font-size:var(--text-sm); color:var(--color-slate-gray);">
          Showing results for "<strong>${fn:escapeXml(searchKeyword)}</strong>"
        </p>
      </c:if>
      ...
      <c:if test="${not empty destinations}">
        <div class="grid-4">
          <c:forEach var="dest" items="${destinations}" varStatus="status">
            <a href="${pageContext.request.contextPath}/destination?id=${dest.id}" style="text-decoration:none; color:inherit; display:block;">
              <article class="card" style="cursor:pointer; transition: transform 0.18s, box-shadow 0.18s;"
                       onmouseover="this.style.transform='translateY(-4px)';this.style.boxShadow='0 8px 32px rgba(0,0,0,0.13)'"
